 ...
 * */
	...
	public static void main(String[] args) throws IOException, ParseException{
		...
		// File input = new File("/home/t-iho/Result/UbiResult/Synthetic"+date+"/all.csv");
		File out = new File("/home/t-iho/Result/trainingdata/simTrajctory"+date+".csv");
		...
		while((line=br.readLine())!=null){
			...
			String id = tokens[0].substring(9);
			
			try{
				if(!id.equals(previd)){
					currentDate = SDF_TS2.parse(currentDay+" 06:00:00").getTime()+random.nextInt(1800000);
					// in case 
					sid = RandomString(4)+id; 
				}
...
//						System.out.println(j);
					...
					if(mode.equals("stay")){
						while(currentDate<SDF_TS2.parse(currentDay+" 06:00:00").getTime()+(slot-11)*1800000){
							currentDate = SDF_TS2.parse(currentDay+" 06:00:00").getTime()+(slot-11)*1800000 + random.nextInt(1800*1000);
						}
						bw.write(sid +"," + SDF_TS2.format(new Date(currentDate))+","+source.getLon()+","+source.getLat()+","+mode);
						bw.newLine();
					}
					if(mode.equals("walk")){
						...
						if(route==null){
							...
							for (Link link : route.listLinks()){
								if(iter.hasNext()){
									Node node = iter.next();
									bw.write(sid +"," + SDF_TS2.format(new Date(currentDate))+","+node.getLon()+","+node.getLat()+","+mode);
									bw.newLine();
								}
								//result
					...
					if(mode.equals("vehicle")){
						...
						if(route==null){
							bw.write(sid +"," + SDF_TS2.format(new Date(currentDate))+","+source.getLon()+","+source.getLat()+","+mode);
							bw.newLine();
							bw.write(sid +"," + SDF_TS2.format(new Date(currentDate))+","+target.getLon()+","+target.getLat()+","+mode);
							bw.newLine();
																						//result
...
//			}
		...
		BufferedWriter bw2 = new BufferedWriter(new FileWriter("/home/t-iho/Result/trainingdata/linkVolume"+date+".csv"));
